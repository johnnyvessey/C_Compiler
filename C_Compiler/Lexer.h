...
struct Token {
	...
	int lineNumber;
	int tokenNumber;
	Token(TokenType type, string value, int lineNumber, int tokenNumber) : type(type), value(value), lineNumber(lineNumber), tokenNumber(tokenNumber) {}
...
struct Lexer {
	...
	static Token ParseToken(const string&& s, int& lineNumber, int& tokenNum);
